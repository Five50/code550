...
/**
 * Get global WordPress theme styles
 * These include CSS custom properties, utility classes, and theme-wide styles
 */
export async function getGlobalStyles(): Promise<string> {
  try {
    ...
    console.warn('Failed to fetch global styles:', error);
    return '';
  }
}

/**
 * Get post-specific styles from WordPress
 * These are custom styles generated by WordPress for specific posts/pages
 * including block variations, custom backgrounds, etc.
 */
export async function getPostStyles(postId: number): Promise<string> {
  try {
    ...
    console.warn(`Failed to fetch styles for post ID ${postId}:`, error);
    return '';
  }
}

/**
 * Determine which WordPress template should be used for a given pathname
 * Returns the WordPress template slug (e.g., 'index', 'single', 'page', 'page-no-title', etc.)
 */
export async function getTemplateForPath(pathname: string): Promise<string> {
  try {
    ...
    if (cleanPath.startsWith('/posts/')) {
      ...
      try {
        ...
        // Return the template assigned to this post, or 'single' as default
        return post.template || 'single';
      } catch {
        return 'single';
      }
    }
    ...
    if (cleanPath === '/') {
      ...
      if (frontPage.type === 'page' && frontPage.content) {
        // Static front page - return its template
        return frontPage.content.template || 'front-page';
      }
      // Blog as homepage
      return 'home';
    }
    ...
    // Remove leading slash and check for page slug
    const pageSlug = cleanPath.substring(1);
    if (pageSlug) {
      try {
