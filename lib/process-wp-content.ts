...
export function processWPContent(html: string): string {
  ...
  processedHtml = processedHtml.replace(
    ...
    (match, beforeClass, classValue, afterClass) => {
      ...
      return `<div${beforeClass}${classValue} *:max-w-3xl${afterClass}>`;
    }
  );
  ...
  processedHtml = processedHtml.replace(
    ...
    (match, classes) => {
      ...
      const constrainedWidthMatch = classes.match(/(?:\[&>?\*\]|\*):max-w-(?:\[720px\]|3xl|\[768px\]|7xl|\[1280px\])/g);
      ...
      if (classes.includes('alignwide')) {
        tailwindClasses.push('!max-w-7xl mx-auto *:max-w-7xl');
      }
      ...
      // Default constrained layout - apply child constraints
        ...
        tailwindClasses.push('*:max-w-3xl');
      }
